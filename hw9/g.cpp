...
// 1) 定义“带整数 key” 的 BST 节点类型（复用前面框架中的 binary_node）
struct int_bst_node {
    int key;
...
// 3) 查找 key=k 的节点，返回其 binary_node*；若未找到返回 nullptr
static binary_node* bst_find_node(binary_tree *tree, int k) {
...
// 4) 获取给定节点的前驱节点：如果其左子树非空，则前驱是左子树中的最大节点；
//    否则沿 parent 指针向上，直到当前节点是其父节点的右子节点，前驱就是该父节点。
//    返回对应的 int_bst_node*，若不存在前驱则返回 nullptr。
static int_bst_node* get_predecessor_node(binary_node *node) {
    if (node == nullptr) {
        return nullptr;
    }
    // 先转换为外层节点类型
    auto curr_int = reinterpret_cast<int_bst_node*>(
        reinterpret_cast<char*>(node) - offsetof(int_bst_node, node)
    );
    // 情况1：如果有左子树，前驱在左子树里
    if (node->child[LEFT] != nullptr) {
        binary_node *sub = node->child[LEFT];
        // 找到左子树中最右的节点
        while (sub->child[RIGHT] != nullptr) {
            sub = sub->child[RIGHT];
        }
        return reinterpret_cast<int_bst_node*>(
            reinterpret_cast<char*>(sub) - offsetof(int_bst_node, node)
        );
    }
    // 情况2：无左子树，沿父链向上寻找
    binary_node *parent = node->parent;
    binary_node *child = node;
    while (parent != nullptr && child == parent->child[LEFT]) {
        child = parent;
    ...
    return reinterpret_cast<int_bst_node*>(
        reinterpret_cast<char*>(parent) - offsetof(int_bst_node, node)
    );
}

// 5) 对外接口：返回关键字 k 的节点前驱。如果 k 不在树中或前驱不存在，则返回 nullptr。
//    假设“树中存在关键字为 k 的结点”，调用时可额外断言或自行检查返回值。
int_bst_node* bst_predecessor(binary_tree *tree, int k) {
    // 先找到 key = k 的节点
    binary_node *found = bst_find_node(tree, k);
    assert(found != nullptr); // 假设调用者保证 k 存在，否则程序终止
    // 再寻找其前驱节点
    return get_predecessor_node(found);
}
