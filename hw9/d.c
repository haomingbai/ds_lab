...
// 1) 定义“带整数 key” 的 BST 节点类型
typedef struct int_bst_node {
    int key;
    binary_node node;   // 嵌入框架的通用二叉树节点
} int_bst_node;

// 2) 比较函数：用于 bst_insert（已由框架调用）和下面的查找
...
// 3) 如果已有 bst_insert，则无需重复。这份代码假定 bst_insert(tree, new_node) 可用，
//    它会用 cmp_int_node 比较并插入新 int_bst_node 到 tree 中。若无，请自行引入。
//    int bst_insert(binary_tree *tree, int_bst_node *new_node);

// 4) 在 BST 中查找关键字 k 时，依次记录访问过的节点 key，最后若找到则返回路径数组。
//    返回值：若成功找到，返回 malloc 出来的 int[] 数组，按从根到目标节点的顺序存放访问过的 key，
//    并通过 out_len 返回路径长度；若没找到，则返回 NULL，且 *out_len = 0。
//    调用者负责 free() 返回的数组。
int *bst_find_path(binary_tree *tree, int k, size_t *out_len) {
    *out_len = 0;
    if (tree == NULL || tree->root == NULL) {
        return NULL;
    }
    // 最多不会超过树中节点总数，先分配一个足够大的临时数组
    size_t maxn = tree->size > 0 ? tree->size : 1;
    int *buffer = malloc(sizeof(int) * maxn);
    if (!buffer) {
    ...
    while (curr) {
        // 访问当前节点，记录它的 key
        int_bst_node *nb = CONTAINER_OF(int_bst_node, node, curr);
        buffer[*out_len] = nb->key;
        (*out_len)++;

        if (k == nb->key) {
            // 找到目标，复制出精确长度的数组并返回
            int *result = malloc(sizeof(int) * (*out_len));
            assert(result);
...
// 以下是一个示例 main，用以演示如何使用上面的 bst_find_path。
// 在实际项目中，可将它作为测试或示例，不属于框架本身。
...
int main(void) {
    ...
    for (size_t i = 0; i < n; i++) {
        int_bst_node *node = malloc(sizeof(int_bst_node));
        assert(node);
        node->key = values[i];
        node->node.parent = node->node.child[LEFT] = node->node.child[RIGHT] = NULL;
        // bst_insert 会使用 cmp_int_node 比较并插入到 tree 中
    ...
    int k = 60;
    size_t path_len = 0;
    int *path = bst_find_path(&tree, k, &path_len);
    if (path) {
        printf("查找 key = %d 的路径（访问过的节点 key）为：", k);
        ...
        printf("BST 中未找到关键字 %d。\n", k);
    }

    // 如果要查找一个不存在的 key
    k = 25;
    path = bst_find_path(&tree, k, &path_len);
    if (path) {
        printf("查找 key = %d 的路径：", k);
        for (size_t i = 0; i < path_len; i++) {
            printf("%d ", path[i]);
        }
        printf("\n");
