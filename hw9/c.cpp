...
typedef struct int_bst_node {
    int key;
    binary_node node;   /* 嵌入通用链表/树节点部分 */
} int_bst_node;

/* 比较函数：返回正数表示 a->key > b->key，负数表示 a->key < b->key，0 表示相等 */
int cmp_int_node(const binary_node *a, const binary_node *b) {
    const int_bst_node *na = CONTAINER_OF(int_bst_node, node, (binary_node *)a);
    const int_bst_node *nb = CONTAINER_OF(int_bst_node, node, (binary_node *)b);
    return (na->key - nb->key);
}

/* BST 插入：调用 binary_tree_insert，一直找下去，直到找到一个 child=NULL 的地方，把 new_node 插上去 */
int bst_insert(binary_tree *tree, int_bst_node *new_node) {
    assert(new_node);
    if (tree->size == 0) {
        /* 直接当作根插入 */
        binary_tree_insert(tree, NULL, &(new_node->node), LEFT);
        tree->size++;
        return 0;
    }
    /* 从 root 开始往下找合适的位置 */
    ...
    while (1) {
        int cmp_res = cmp_int_node(curr, &(new_node->node));
        if (cmp_res == 0) {
            /* 不允许插入相同 key */
            return -1;
        } else if (cmp_res > 0) {
            /* curr->key > new_node->key，往左走 */
            if (curr->child[LEFT] == NULL) {
                binary_tree_insert(tree, curr, &(new_node->node), LEFT);
                tree->size++;
                return 0;
            } else {
                curr = curr->child[LEFT];
            ...
            /* curr->key < new_node->key，往右走 */
            if (curr->child[RIGHT] == NULL) {
                binary_tree_insert(tree, curr, &(new_node->node), RIGHT);
                tree->size++;
                return 0;
            } else {
                curr = curr->child[RIGHT];
...
/* 逆中序遍历：先遍历右子树，再访问当前结点，再遍历左子树 */
void print_descending(binary_node *root) {
    if (root == NULL) return;

    /* 先递归打印右子树 */
    print_descending(root->child[RIGHT]);

    /* 再打印 “当前结点” 对应的 key */
    int_bst_node *me = CONTAINER_OF(int_bst_node, node, root);
    printf("%d ", me->key);

    /* 最后递归打印左子树 */
    print_descending(root->child[LEFT]);
}

/* 对外接口：给定整棵树，按递减顺序输出并换行 */
void bst_print_all_desc(binary_tree *tree) {
    if (tree == NULL || tree->root == NULL) {
        printf("(empty)\n");
        return;
    }
    print_descending(tree->root);
    printf("\n");
}

int main(void) {
    /* 1) 创建并初始化一棵空的 binary_tree */
    binary_tree tree;
    init_binary_tree(&tree);

    /* 2) 准备一些要插入的整数 */
    int values[] = { 35, 12, 99, 47, 23, 76, 12, 5, 88, 47 };
    size_t n = sizeof(values) / sizeof(values[0]);

    /* 3) 按顺序插入（对重复 key 插入会被忽略） */
    for (size_t i = 0; i < n; i++) {
        int_bst_node *node = (int_bst_node *)malloc(sizeof(int_bst_node));
        assert(node);
        node->key = values[i];
        node->node.parent = node->node.child[LEFT] = node->node.child[RIGHT] = NULL;
        /* bst_insert 会检查重复，如果重复则返回 -1，我们简单忽略 */
        bst_insert(&tree, node);
    }

    /* 4) 现在按“递减”顺序输出所有不重复的 key */
    printf("BST 中所有节点的 key（从大到小）为：\n");
    bst_print_all_desc(&tree);

    /* 5) 释放内存（这里只做演示，实际项目中要对整棵树做后序遍历释放） */
    /*  为了简洁，这里不做完整释放，仅示意： */
    // … 遍历 tree.root，把每个 CONTAINER_OF(int_bst_node, node, 某 binary_node*) free(…)。
    // 省略…
