...
// 3) 对外接口：在 BST 中查找“第一个值大于 k 的节点”（即最小的 key > k）
//    返回对应的 int_bst_node*；若不存在则返回 nullptr。
//    时间复杂度 O(height)，利用 BST 性质沿路径搜索。
int_bst_node* bst_find_first_greater(binary_tree *tree, int k) {
    if (tree == nullptr || tree->root == nullptr) {
        return nullptr;
    }
    binary_node *curr = tree->root;
    int_bst_node *candidate = nullptr;
    ...
    while (curr != nullptr) {
        ...
        if (nb->key > k) {
            // curr 有资格成为答案，记录并向左子树继续找更小的“仍大于 k”的值
            candidate = nb;
            curr = curr->child[LEFT];
        } else {
            // nb->key <= k，答案必定在右子树
