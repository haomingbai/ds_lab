...
// 1) 定义“带整数 key” 的 BST 节点类型（复用上次框架）
struct int_bst_node {
    int key;
    binary_node node;   // 嵌入通用二叉树结点
};

// 2) 比较函数：用于 bst_insert（已由框架调用）
static int cmp_int_node(const binary_node *a, const binary_node *b) {
    // C++ 下必须用 reinterpret_cast 结合 offsetof 来做 CONTAINER_OF
...
// 3) 辅助递归函数：中序遍历时收集最小的 m 个 key
static void inorder_collect(binary_node *root, size_t &remaining, std::vector<int> &out) {
    if (root == nullptr || remaining == 0) {
        return;
    }
    // 先遍历左子树
    inorder_collect(root->child[LEFT], remaining, out);
    if (remaining == 0) {
        return;
    }
    // 访问当前结点，转换到外层 int_bst_node*
    auto node_ptr = reinterpret_cast<int_bst_node*>(
        reinterpret_cast<char*>(root) - offsetof(int_bst_node, node)
    );
    out.push_back(node_ptr->key);
    --remaining;
    if (remaining == 0) {
        return;
    }
    // 然后遍历右子树
    inorder_collect(root->child[RIGHT], remaining, out);
}

// 4) 对外接口：返回 BST 中前 m 小的 key 序列（按升序排列）
//    若 m 大于树中节点数，则只返回所有节点的 key。
std::vector<int> bst_get_smallest(binary_tree *tree, size_t m) {
    std::vector<int> result;
    if (tree == nullptr || tree->root == nullptr || m == 0) {
        return result;
    }
    // 最多不会超过 tree->size
    result.reserve(m < tree->size ? m : tree->size);
    size_t remaining = (m < tree->size ? m : tree->size);
    inorder_collect(tree->root, remaining, result);
    return result;
}
...
int main() {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // 框架函数 bst_insert 会调用 cmp_int_node 进行比较
        bst_insert(&tree, &node->node, cmp_int_node);
    }

    // 输出前 5 小的节点 key
    size_t m = 5;
    auto smallest = bst_get_smallest(&tree, m);
    std::cout << "BST 中前 " << m << " 小的 key（按升序）为：";
    for (int k : smallest) {
        std::cout << k << ' ';
    }
    std::cout << '\n';

    // 如果 m > 树中节点数，也会返回所有 key
    m = 20;
    auto all_keys = bst_get_smallest(&tree, m);
    std::cout << "当 m = " << m << " 时，输出所有 key：";
    for (int k : all_keys) {
        std::cout << k << ' ';
    }
    std::cout << '\n';
