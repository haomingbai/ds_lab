...
bool is_complete_binary_node(binary_node *root) {
  ...
  size_t hleft = binary_node_height(root->child[LEFT]);
  size_t hright = binary_node_height(root->child[RIGHT]);
  if ((hright > hleft) || (hleft - hright) > 1) {
    return false;
  }

  bool status_left = is_complete_binary_node(root->child[LEFT]);
  bool status_right = is_complete_binary_node(root->child[RIGHT]);
